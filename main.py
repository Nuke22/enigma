def choose_swap (swaps_allowed = 3):
    add_swap = True
    swap_array = []
    swaps_done = 1
    # rewrite this annoyance!
    while add_swap and swaps_done <= swaps_allowed:
        keep_swaping = input(f"\nDo you want to add aditional letter change? ({swaps_done}/{swaps_allowed}) (y/n): ")
        
        if keep_swaping.lower() == "yes" or keep_swaping.lower() == "y":
            swap_pair = input("Please enter what pair of letter you want to swap? (Remember, you can not mention the same letter twice!): ")
